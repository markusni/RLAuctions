...

{
  ...
  agentbid.func<-function(exp.df,agentval,agentsd){
    ...
    if(as.character(roundresult[[2]])==1||roundresult[[5]][1]==T||nextminaction>1.4){
    ...
    scaled <- as.data.frame(scale(t(as.data.frame(currstate[c(2,4,6:16)])), center = vtmins, scale = vtmaxs - vtmins))
    vt_estimate<-unname(round(as.vector(compute(vt_nn,scaled)$net.result)*(vtmaxs_vt-vtmins_vt)+vtmins_vt,digits=1))
    
    compstate<-c(currstate[c(1,2,4,16,3,5)],vt_estimate)
    ...
    if(runif(1,min=0,max=1000)<=epsilon*1000){
      ...
      exploreopts<-c(lastaction,exploreopts[exploreopts>=nextminaction])
  ...
  actionlearn.func<-function(exp.df){
    ...
    for(i in 1:tracer_l){
      ...
      update<-tracer[[tracer_l-i+1]][c(1,2,4,6:16,3,5,4,17)]
      
      #newtarget<-resagent/(1.5-(alpha/2))^(i-1)
      #Ein temp exp.df wird erstellt, wo nur die Vorhersagerelevanten Faktoren drin sind
      rel_exp.df<-exp.df[,c(1:2,15:18)]
      
      #Suche nach Vorerfahrungen für die Updates, bei Verlusten werden keine Ähnlichen States angepasst
      # if(newtarget>=0){
      similarities<-similar.func(tempmatrix = rel_exp.df[1:amdata,],currstate=update[c(1,2,15:18)])
      ...
      if(as.character(length(relevantstates))>=1){
        ...
        exp.df$Result[relevantstates]<-exp.df$Result[relevantstates]+round((1/((similarities[[2]]+1)^(13-alpha*13)))*(newtarget-exp.df$Result[relevantstates]),digits=3)
  ...
  vtlearn.func<-function(exp.df,agentval){
    ...
    for(i in 1:tracer_l){
      ...
      update<-c(tracer[[i]][c(1,2,4,6:16)],vtrue)
      ...
      similarities<-similar.func(tempmatrix = exp.df[relevantstates,c(startcol:(startcol+n),startcol+12)],currstate=update[c(2:(n+2),14)])
  ...
  vt_updates.func<-function(exp.df){
    ...
    tempexp.df<-exp.df[1:amdata,c(2:14)]
  ...
  create_nn.func<-function(exp.df,type){
    ...
    predictions<-compute(newnn,testing.df[,1:(ncol(testing.df)-1)])
    
    predictions<-predictions$net.result*(max(exp.df[,targetcol])-min(exp.df[,targetcol]))+min(exp.df[,targetcol])
    
    realv<-exp.df[-index,length(cols)]
    
    modelsd <- sqrt(sum((realv - predictions)^2)/length(realv))
...
###########################################################
{
actionexp.df<-setNames(data.frame(matrix(ncol = 20, nrow = 10000)), c("Nplayers","Round","AgentVal",paste("Maxbid", 2:11,sep=""),"Globalmaxbid","Minbidinc","AgentSD","VtEstimator","ActionRel","Result","NUpdates"))
vt_estimator.df<-setNames(data.frame(matrix(ncol = 16, nrow = 10000)), c("Nplayers","Round","AgentVal",paste("Maxbid", 2:11,sep=""),"Globalmaxbid","Vtrue","NUpdates"))
...
while(auc_counter%%100!=0 || balance.df[1,2]<=max(balance.df[-1,2])){
  ...
  if(auc_counter%%100==0){
    ...
    print(cor(vt_estimator.df[1:amdata_vt,-c(15,16)],vt_estimator.df$Vtrue[1:amdata_vt]))
    
    timeystats.mtr<-rbind(timeystats.mtr,c(auc_counter,t(balance.df)[2,]))
    
    plot(timeystats.mtr[,1:2],ylab="Balance (Agent)",xlab="Auction")
  }
  ...
  if(auc_counter%%NNUpdateCycle==0){
    ...
    vt_nn<-create_nn.func(exp.df=vt_estimator.df[,2:15],type="vt_nn")
    ...
    scaledRes<-sqrt(abs(actionexp.df$Result[1:amdata]))
    scaledRes[actionexp.df$Result[1:amdata]<0]<-scaledRes[actionexp.df$Result[1:amdata]<0]*(-1)
    
    action_nn<-create_nn.func(exp.df=cbind(actionexp.df[1:amdata,c(1:3,14:18)],scaledRes),type="action_nn")
...
sd(actionexp.df$Result[1:amdata])
test<-sqrt(abs(actionexp.df$Result[1:amdata]))
sd(test)
test[actionexp.df$Result[1:amdata]<0]<-test[actionexp.df$Result[1:amdata]<0]*(-1)
