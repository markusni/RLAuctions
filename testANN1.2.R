...

{
  ...
  agentlearn.func<-function(exp.df){
    #exp.df<-agentexp.df
    ...
    for(i in 1:tracer_l){
      ...
      if(as.character(length(relevantstates))>=1){
        ...
        exp.df$Result[relevantstates]<-exp.df$Result[relevantstates]+round((alpha-similarities[[2]])*(newtarget-exp.df$Result[relevantstates]),digits=3)
  ...
  update_nn.func<-function(exp.df){
...
###########################################################
{
agentexp.df<-setNames(data.frame(matrix(ncol = 19, nrow = 10000)), c("Nplayers","Round","Minbidinc","AgentVal","AgentSD",paste("Maxbid", 2:11,sep=""),"Globalmaxbid","Action","Result","NUpdates"))
...
while(auc_counter%%100!=0 || balance.df[1,2]<=max(balance.df[-1,2])){
  ...
  agentexp.df<-agentlearn.func(exp.df=agentexp.df)
  ...
  if(auc_counter%%2000==0){
    
    cat("\n","UPDATING NN...","\n")
    
    currnn<-update_nn.func(exp.df = agentexp.df)
...
(length(agentexp.df$Result[which(agentexp.df$Action<=0.05)])>0&&any((as.character(agentexp.df$Result[which(agentexp.df$Action<=0.05)])<0)==T))

which(agentexp.df$Action<=0.05)[which((agentexp.df$Result[which(agentexp.df$Action<=0.05)]<0)==T)]
...
test.df<-agentexp.df
amdata<-match(NA, test.df[,1],nomatch = (nrow(test.df)+1))-1
