...
#####################################################FUNCTIONS####
{
  ...
  calc_epsilon.func<-function(timeystats.mtr){
    ...
    balance_diffs<-c(0,diff(timeystats.mtr[,2]))
    ...
    return(round(0.51-(0.5-(200/(auc_counter+400)))*scaled,digits=3))
  }
  
  defnextnaivebid.func<-function(n,highroller,lastbids,globalmaxbid,val,sd,realprofits){
    if(changebehav==T){
      realprofits<-0.2
    }
    ...
    nextminbid<-globalmaxbid*1.05
    playermaxbid<-val-(qnorm(realprofits^(1/(n-1))))*sd
    
    player2maxbid<-playermaxbid[1]
    #if(changebehav==F){
      playermaxbid[1]<-0.9*val[1]
    #}
    playermaxbid<-c(player2maxbid,playermaxbid[-1])
    
    nextbids<-ceiling(((playermaxbid-nextminbid)*(runif(n-1,min=0.1,max=0.99)^4)+nextminbid)*10)/10
    if(nextminbid<playermaxbid[1])nextbids[1]<-ceiling(nextminbid*10)/10
    
    optout<-which(nextbids<nextminbid)
    nextbids[optout]<-lastbids[optout]
    nextbids[highroller]<-lastbids[highroller]
    return(list(nextbids,round(max(playermaxbid),digits=2)))
  }
  ...
  agentbid.func<-function(exp.df,agentval,agentsd){
    ...
    if(as.character(roundresult[[2]])==1||roundresult[[5]][1]==T||nextminaction>1.2){
    ...
    if(auc_counter<=NNUpdateCycle){
      ifelse(nextminaction>1.2,action<-lastaction,action<-nextminaction)
  ...
  create_nn.func<-function(exp.df,type){
    ...
    if(exists(type)){
      ...
      set_weights<-c(set_weights,runif(2*length(set_weights),min=1,max=3))
      newnn<-neuralnet(formula,data=training.df,hidden=c(5,3),stepmax = 50e+04,linear.output=T,rep=3,startweights = set_weights, lifesign = "full")
      which.min(newnn$result.matrix[1,])
    }else{
      newnn<-neuralnet(formula,data=training.df,hidden=c(5,3),stepmax = 25e+04,linear.output=T,rep=10,lifesign = "full")
    }
...
plot(1:64,amdatastats.mtr[,1])
#####################################################USER SET#####
{
  minval<-2
  maxval<-100
  alpha<-0.1 #Lernrate [0,1], je hoeher desto, schneller lernt der Agent, vermischt allerdings auch staerker Erfahrungen aus verschiedenen States miteinander
  resprice<-1
  NNUpdateCycle<-400
  realprofits<-0.5
}
...
#####################################################PRESET#####
{
  ...
  agentstats.mtr<-setNames(data.frame(matrix(ncol = 6, nrow = 0)), c("Auction","Vt","Maxplayerbid","Agentval","Agentbid","Winamount"))
  amdatastats.mtr<-setNames(data.frame(matrix(ncol = 1, nrow = 0)), "AmountData")
  auc_counter<-0
  currdev_vt<-NA
  currdev_action<-NA
  next_epsilon<-NA
...
#for(i in 1:100){
  ...
  while(roundresult[[1]]==F) {
    ...
    agentbiddetails<-agentbid.func(exp.df=actionexp.df,agentval=currenv[[2]][1],agentsd=currenv[[3]][1])
    
    playersnextbids<-defnextnaivebid.func(n,highroller = roundresult[[2]],lastbids,globalmaxbid,val=currenv[[2]][-1],
                                          sd=currenv[[3]][-1],realprofits)
    
    newbids<-c(agentbiddetails[[1]],playersnextbids[[1]])
    
    #Auktionator beurteilt
    roundresult<-auctioneer.func(highroller = roundresult[[2]],valtrue = currenv[[1]])
    
    #if(roundresultx[[1]]==T&&max(playersnextbids[[1]])<playersnextbids[[2]])break
    #roundresult<-roundresultx
    
    #tracer wird ueberschrieben solange der Agent noch drin ist
    if(roundresult[[5]][1]==F||as.character(bidround)==1){
...
################STATISTICS####
  ...
  agentstats.mtr[nrow(agentstats.mtr)+1,]<-c(auc_counter,currenv[[1]],round(playersnextbids[[2]],digits=2),currenv[[2]][1],roundresult[[6]][1],ifelse(roundresult[[2]]==1,roundresult[[3]],0))
  
  ######OPTIMUM NOTIFICATION
  {
  relactions<-seq(0,1.2,0.01)
  bidrange<-relactions*currenv[[2]][1]
  
  winbidrange<-round(bidrange[bidrange>(playersnextbids[[2]]/1.05)&bidrange<currenv[[1]]],digits=1)
  relactions<-relactions[bidrange>(playersnextbids[[2]]/1.05)&bidrange<currenv[[1]]]
  
  if(length(winbidrange)){
    wincounter<-wincounter+1
    cat("\n","\n","In Auktion ",auc_counter," waeren Gewinne i.H.v. ",round((currenv[[1]]-min(winbidrange)),digits=2),"-",
        round((currenv[[1]]-max(winbidrange)),digits=2)," moeglich mit Geboten ",min(winbidrange),"-",max(winbidrange),
        "\n"," (Relativ: ",min(relactions),"-",max(relactions),", bei Val: ",currenv[[2]][1]," und vt ",currenv[[1]],")","\n",
        "Counter: ",wincounter,"\n",sep="")
  }
  }
  ...
  if(auc_counter%%100==0){
    ...
    amdatastats.mtr[nrow(amdatastats.mtr)+1,1]<-amdata
    ...
    #Agenten wechseln bei BalanceP2<BalanceAgent ihr Verhalten
    if(min(balance.df[3:11,2])<balance.df[1,2])changebehav<-T
  }
