...

{
  ...
  maximizeq.func<-function(nn, state, maxs, mins, nextminaction, lastaction){
    ...
    minreward<-mins[names(mins)=="scaledQ"]
    maxreward<-maxs[names(maxs)=="scaledQ"]
    ...
    possactions<-c(lastaction,seq(nextminaction_c,1.4,0.1))
  ...
  vtlearn.func<-function(exp.df,agentval){
    ...
    if(as.character(roundresult[[2]])==1){
      ...
      if(as.character(tracer[[1]][[15]])==0)vtrue<-agentval
    }else{
      return(exp.df)
    }
  ...
  actionlearn.func<-function(exp.df){
    ...
    for(i in 1:tracer_l){
      ...
      if(as.character(length(relevantstates))>=1){
        ...
        #Anpassung der nun uebrigen States
        exp.df$QReward[relevantstates]<-round(exp.df$QReward[relevantstates]+(1/((similarities[[2]]+1)^(13-alpha*13)))*(newtarget-exp.df$QReward[relevantstates]),digits=4)
        exp.df$NUpdates[relevantstates]<-exp.df$NUpdates[relevantstates]+1
