...

{
  ...
  agentbid.func<-function(exp.df,agentval,agentsd){
    ...
    if(as.character(roundresult[[2]])==1||roundresult[[5]][1]==T||nextminaction>1.3){
    ...
    if(auc_counter<=NNUpdateCycle){
      ifelse(nextminaction>=(0.85+bidround*0.05),action<-lastaction,action<-round(runif(1,min=nextminaction,max=(0.85+bidround*0.05)),digits=2))
    ...
    compstate<-currstate[c(1:3,14:17)]
    ...
    if(runif(1,min=0,max=1000)<=epsilon*1000&&(ceiling(nextminaction*100)/100)<=1){
      explore<-1
      ...
      exploreopts<-c(lastaction,seq(ceiling(nextminaction*100)/100,1.2,0.01))
  ...
  maximizeq.func<-function(nn, state, maxs, mins, nextminaction, lastaction, agentval){
    ...
    possactions<-c(lastaction,seq(nextminaction_c,1.3,0.1))
  ...
  vtlearn.func<-function(exp.df,agentval){
    ...
    for(i in 1:tracer_l){
      ...
      if(length(relevantstates)>0){
        ...
        exp.df$Vtrue[relevantstates]<-round(exp.df$Vtrue[relevantstates]+(1/((similarities[[2]]+1)^(13-alpha*13)))*alpha*(vtrue-exp.df$Vtrue[relevantstates]),digits=3)
  ...
  actionlearn.func<-function(exp.df){
    ...
    rel_exp.df<-exp.df[1:amdata,c(startcol:(startcol+2),(startcol+13):(startcol+17))]
    ...
    for(i in 1:tracer_l){
      ...
      if(length(relevantstates)>1){
        #Anpassung der nun uebrigen States
        exp.df$QReward[relevantstates]<-round(exp.df$QReward[relevantstates]+(1/((similarities[[2]]+1)^(13-alpha*13)))*alpha*(newtarget-exp.df$QReward[relevantstates]),digits=4)
        exp.df$NUpdates[relevantstates]<-exp.df$NUpdates[relevantstates]+1
        
        #Neue States angelegt mit willkuerlichem Grenzwert fuer Aehnlichkeit 0.05%
        if(any(similarities[[2]]<=0.05)==F){
          update[19]<-round(alpha*newtarget,digits=4)
          exp.df[amdata+1,]<-c(update,1)
          new_states<-c(new_states,amdata+1)
        }
  ...
  similar.func<-function(tempmatrix, currstate, maxs, mins){
    ...
    hits<-unname(which(apply(distances,1,function(x)all(x<=0.1))))
  ...
  create_nn.func<-function(exp.df,type){
    ...
    if(exists(type)){
      ...
      set_weights<-c(set_weights,runif((97*9),min=1,max=3))
      newnn<-neuralnet(formula,data=training.df,hidden=c(5,3),stepmax = 1e+06,linear.output=T,rep=10,startweights = set_weights, lifesign = "full")
      which.min(newnn$result.matrix[1,])
    }else{
      newnn<-neuralnet(formula,data=training.df,hidden=c(5,3),stepmax = 1e+06,linear.output=T,rep=10,lifesign = "full")
    }
...
while(auc_counter%%100!=0 || balance.df[1,2]<=max(balance.df[-1,2])){
  ...
  if(auc_counter%%NNUpdateCycle==0){
    ...
    NNUpdateCycle<-round(NNUpdateCycle*1.5,digits=1)
    ...
    #scaledQ<-round(sqrt(abs(actionexp.df$QReward[1:amdata])),digits=4)
    ...
    action_nn<-create_nn.func(exp.df=actionexp.df[1:amdata,c(1:3,14:19)],type="action_nn")
